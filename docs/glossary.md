...
| **Managed unit** *(control-plane sense; keyed by `env_id`)* | One vended **fabric or spoke** the control plane records and tracks in the Postgres registry — addressed by `env_id` or its natural key `(kind, subscription, name)`, surfaced by the MCP tool `ShowEnvironment`. Spec 006 historically calls this row an "environment"; that is the looser control-plane use of the word and is **not** the workload **Environment** above. |
| **env_id** | The control plane's stable surrogate identifier (UUIDv7) for one **managed unit** (a fabric or spoke) in the Postgres registry — the primary key of intent and the **correlation key** threaded through a dispatched workflow (embedded in the run name) so a `workflow_run` outcome can be matched back to the request that caused it. Distinct from the human natural key `(kind, subscription, name)` that drives idempotent convergence. Introduced by the action layer (spec 006). *(The thing it identifies is a managed unit, not the workload **Environment** — see both entries above.)* |
