...
| **Environment** *(workload sense)* | A named **workload** instance grouping (e.g., `dev`, `demo`), carried by the `pdp-env` tag; "what workload environments do I have deployed?" is answered by inventory grouped on this (MCP tool `ListWorkloadEnvironments`). **Distinct from a managed unit** (below): a vended fabric/spoke is *not* a workload environment, and this list stays empty until workloads (spec 008) exist. |
| **Managed unit** *(control-plane sense; keyed by `env_id`)* | One vended **fabric, spoke, or workload** the control plane records and tracks in the Postgres registry — addressed by `env_id` or its natural key `(kind, subscription, name)`, surfaced by the MCP tool `ShowEnvironment`. Workload rows joined the registry with spec 008 (note the natural key: workload names are unique per *subscription*, not per spoke). Spec 006 historically calls this row an "environment"; that is the looser control-plane use of the word and is **not** the workload **Environment** above. |
| **env_id** | The control plane's stable surrogate identifier (UUIDv7) for one **managed unit** (a fabric, spoke, or workload) in the Postgres registry — the primary key of intent and the **correlation key** threaded through a dispatched workflow (embedded in the run name) so a `workflow_run` outcome can be matched back to the request that caused it. Distinct from the human natural key `(kind, subscription, name)` that drives idempotent convergence. Introduced by the action layer (spec 006). *(The thing it identifies is a managed unit, not the workload **Environment** — see both entries above.)* |
...
| **Archetype catalog** | The registry of deployable workload archetypes: a repo-managed declarative file (`archetypes/catalog.json`, changed only by PR) projected into Postgres tables by the control plane at startup — per version: module path, pinned git tag, parameter JSON schema. A verb can only stamp what the catalog lists; chat/CLI can never alter it. |
