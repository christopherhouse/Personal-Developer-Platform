...
- [X] T027 [US1] Run spec-006 quickstart **T071/T072** against the hosted control plane (live spoke vend/destroy on `westus3`, tracked to terminal) — quickstart Scenario 7 — **DONE (live, 2026-06-24 → 2026-07-01)**: after the Phase 9 async plan→confirm→apply fix landed, full chat-driven vend→destroy succeeded against the hosted control plane — workflow-run record shows `spoke-vend` `pdp plan`+`pdp apply` succeeded 2026-06-24 and 2026-06-25, `spoke-destroy` succeeded 2026-06-25 and 2026-07-01, all tracked to terminal. **Closes spec-006 T071/T072**; shared acceptance with T040/T041.
...
- [X] T038 [US2] Run the one-time bootstrap for `uami-mcp` (`pgaadauth_create_principal_with_oid` + `GRANT` on `ipam`+`registry`). **DONE (live)**: the mcp principal was bootstrapped alongside `uami-api` — same evidence as T025 (PR #31 idempotent bootstrap self-healing 28P01, #30 app-reg bootstrap), confirmed by the MCP node authenticating to the ledger and serving tool calls live (PR #32 fixed a serialization bug found by invoking a tool through the deployed MCP). Manual fallback (the `pgaadauth_bootstrap_uami_mcp` tofu output) remains documented.
- [X] T039 [US2] Deploy (images + `tofu apply`) and verify the MCP auth gate live — quickstart Scenario 2. **DONE (live)**: the `mcp` app is deployed behind the YARP ingress and the Entra OAuth gate is verified — PR #30 corrected the MCP OAuth config (tenant + audience) against the live endpoint, and #32 fixed a tool-result bug found by calling a tool *through* the authenticated gate. (The live vend/destroy *through* the gate — T040/T041 — remains blocked by the Phase 9 plan→apply fix.)
- [X] T040 [US2] Live: vend a spoke through an MCP client with the plan surfaced before apply, tracked to terminal — quickstart Scenario 3 — **DONE (live): validated by owner (marked 2026-07-01)**. **also the acceptance for the Phase 9 amendment**: `PlanSpokeVend` returns a token immediately without blocking; a `ShowEnvironment`/`RunStatus` read surfaces the real plan once it completes; `ApplySpokeVend` before the plan finishes returns the distinct "plan not ready" message (NOT single-flight) and does NOT consume the token; the confirmed apply tracks to terminal (SC-012). Verify it works with the Api node's background reconciler disabled.
- [X] T041 [US2] Live: destroy the spoke — refused without token + verbatim name, then succeeds (allocation released, env `destroyed`) — quickstart Scenario 4 — **DONE (live): validated by owner (marked 2026-07-01)**. **also verifies the Phase 9 amendment for the destroy flow**: `PlanSpokeDestroy` returns a token immediately; `DestroySpoke` before the destroy-plan finishes returns "plan not ready" (token preserved); the confirmed destroy tracks to terminal.
...
- [X] T047 [US3] Live: ask "what's deployed?" / IPAM / "what did I ask, what happened?" and verify correct sourcing — quickstart Scenario 5 — **DONE (live, 2026-07-01)**: exercised through the deployed MCP with an owner token — `whats_deployed` (Resource Graph inventory + drift + subscription coverage), `query_ipam` (ledger), `run_history`/`show_environment` (registry audit with OpenTofu plan summaries). Correct sourcing confirmed (inventory→ARG, CIDRs→ledger, intent/audit→registry). Env ref format is `kind:subscription:name`.
...
- [X] T050 [US4] Deploy and verify `env_id`-correlated logs + traces land in App Insights (verb → dispatch → run-state transitions → terminal) — quickstart Scenario 6 — **DONE (live, ~2026-07-01, owner-verified)**: validated alongside the T027/T047 live acceptance — `env_id`-correlated traces observed end-to-end in the workspace-based App Insights across verb → dispatch → run-state transitions → terminal.
...
- [ ] T054 [US5] Verify (live) the dispatched destroy removes all spec-7 resources (ACA env+apps, 3 UAMIs, ACR, Log Analytics, App Insights, Key Vault) — quickstart Scenario 8 — **DEFERRED (recorded 2026-07-02)**: the teardown *machinery* is complete and merged (gated `controlplane-host-destroy.yml` T052 + cleanup runbook T053), but the live destroy run is intentionally deferred — the host stack **is** the live control plane, and specs 8/9 depend on it staying up; destroying it now to prove destroyability would take the platform down for a checkbox. Verification rides on the first spec that rebuilds or migrates the host stack (at latest spec 9 multi-region). Deferral recorded in `docs/spec-backlog.md` (same pattern as the spec-004 Gate-G1 and spec-006 T071/T072 deferrals).
- [ ] T055 [US5] Verify via Resource Graph that the `prevent_destroy`+`CanNotDelete` ledger RG is **untouched** and no dangling role grants / orphaned Entra registrations remain (SC-010) — **DEFERRED (recorded 2026-07-02)**: post-destroy verification; rides on T054's deferred live destroy.
...
- [X] T059 Execute the full `quickstart.md` (Scenarios 0–8) end-to-end against `westus3` — **DONE (Scenarios 0–7, live, 2026-06-2x → 2026-07-01)**: deploy + bootstrap (T025/T026), auth gate (T039), chat vend→destroy with the plan→confirm gate (T027/T040/T041), conversational reads (T047), telemetry trace (T050) all executed against the live `westus3` deployment. **Scenario 8 (teardown) deferred with T054/T055** — see the recorded deferral there.
...
- [X] T069 [US2] Update the mutate/destroy tools in `src/Pdp.Mcp/Tools/SpokeTools.cs` + `FabricTools.cs`: call `tokens.Check(...)` (no consume) → invoke the verb → `tokens.Consume(...)` **only on a successful gated dispatch**; map `PlanNotReadyException`/`PlanFailedException` to clear, retryable `McpException` messages ("plan hasn't finished yet — check its status with ShowEnvironment/RunStatus and retry" / "the plan failed; nothing to apply"); the token is **not** consumed on those paths. `Plan*` tools stay pure dispatch-and-return.
- [X] T070 [US2] Update tool `[Description]`s in `SpokeTools`/`FabricTools` to state the **plan → check status (ShowEnvironment/RunStatus) → apply** sequence explicitly (so the agent reviews the real plan and never skips straight from plan to apply), and that `Apply*`/`Destroy*` return "plan not ready" until the plan run has succeeded — surfacing the async contract to the model.
- [X] T071 [US2] Run `dotnet build` (0 warn/0 err), `dotnet test` (Pdp.Mcp.Tests + Pdp.ControlPlane.Verbs.Tests green incl. T062–T064), `dotnet format` clean. Confirm no prohibited deps introduced and the verb-layer single-flight tests (spec-006) still pass unchanged.
- [X] T072 [US2] Update the design-artifact cross-references already amended (`spec.md` FR-018/019/020 + SC-012, `contracts/mcp-tool-surface.md`, `research.md` §12/§13, `data-model.md` §5) are reflected in code comments/READMEs where the old "~5-min TTL / removed on first validation / no in-tool polling" wording appears (`src/Pdp.Mcp/README.md`, XML docs on the tools + token service); no stale contradiction remains. Live verification rides on **T040/T041**.
